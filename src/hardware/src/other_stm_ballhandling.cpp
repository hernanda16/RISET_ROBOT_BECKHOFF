...
void callbackRoutine(const ros::TimerEvent&)
{
    ...
    case 0:
        ...
        if (force_dribble_control == 4) {
            sp_buffer[0] = (fabsf(v_robot[1]) * 20);
            sp_buffer[1] = -(fabsf(v_robot[1]) * 20);
        } else {
            if (stand_by) {
                sp_buffer[0] = -4000;
                sp_buffer[1] = 4000;
    ...
    case 12: {
        ...
        if (force_dribble_control == 2) {
            ...
            sp_buffer[0] = -1000; // 1500 vs MR.DEV
            sp_buffer[1] = 1000; // 1500 vs MR.DEV
    ...
    case 2: {
        ...
        if (v_robot[0] > 5) {
            sp_buffer[4] += (fabsf(v_robot[0]) * 36.5); // kanan hisap dipercepat
        ...
        if (v_robot[1] > 5) {
            sp_buffer[4] += (fabsf(v_robot[1]) * 20);
            sp_buffer[5] -= (fabsf(v_robot[1]) * 20);
        }
        if (v_robot[1] < -5) {
            sp_buffer[4] -= (fabsf(v_robot[1]) * 40);
            sp_buffer[5] += (fabsf(v_robot[1]) * 40);
        }
...
void callbackSubscribersub_v_robot_actuation(const geometry_msgs::Pose2DConstPtr& msg)
{
    ...
    v_robot[0] -= (v_robot[2] * DEG2RAD * 25);
}
