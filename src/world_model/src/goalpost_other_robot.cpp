...
void loadConfig()
{
    ...
    center_cam_x = config["Camera"]["cam_offset_x"].as<int16_t>();
    center_cam_y = config["Camera"]["cam_offset_x"].as<int16_t>();
...
void callbackRoutine(const ros::TimerEvent& event)
{
    ...
    static const float keeper_pos_on_field[2] = { 400, 1200 };
    ...
    gawang_kiri.field_x = 300;
    gawang_kiri.field_y = 1200;
    gawang_kanan.field_x = 500;
    gawang_kanan.field_y = 1200;
    ...
    if (empty_goal_contour.size()) {
        ...
        if (error_sudut_ke_tengah < 0) {
            int x_offset = (x_kiri - frame_x_target_2024) * 0.4;
            ...
            int x_offset = (x_kanan - frame_x_target_2024) * 0.4;
