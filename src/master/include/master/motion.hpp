...
public:
    ...
    uint8_t hadapGawangVisionPivot(float target_x, float target_y, uint16_t radius_point, uint16_t minmax_vel_linear, uint16_t minmax_vel_angular, uint8_t flag = 0);
    ...
    obstacle_checking_t obstacleCheckNearestTo(float theta, float tolerance = 25, float dist = 100, float to_x = 0, float to_y = 0);
