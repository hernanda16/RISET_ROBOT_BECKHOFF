...
mainApp.controller("mainAppController", function ($scope, $interval) {
  ...
  $scope.frame_potential_2d = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/passing_point_grapher&type=ros_compressed";
  ...
  $scope.sub_toggle_near_goal = new ROSLIB.Topic({
    ros: $scope.ros,
    name: "near_goal_toggle",
    messageType: "std_msgs/UInt8"
  });

  $scope.pub_toggle_near_goal = new ROSLIB.Topic({
    ros: $scope.ros,
    name: "near_goal_toggle",
    messageType: "std_msgs/UInt8"
  });

  $scope.sub_toggle_near_goal.subscribe((msg) => {
    let myToggle = angular.element(document.querySelector('#toggleNearGoal'));
    if (Number(msg.data) === 1) {
      myToggle.attr('checked', 'checked');
      $scope.state_near_goal = "NEAR GOAL ON";
      $scope.checkedToggleNearGoal = true;
    } else {
      myToggle.removeAttr('checked');
      $scope.state_near_goal = "NEAR GOAL OFF";
      $scope.checkedToggleNearGoal = false;
    }
  });

  $scope.toggleNearGoal = function (e) {
    console.log(e.checked);
    if (e.checked) {
      checkedToggleNearGoal = 1;
      $scope.msg_vision_main_data_on.data = 1;
      $scope.exportClickedMain();
      $scope.variablechange = "NEAR GOAL ON";
      console.log($scope.msg_vision_main_data_on);
      $scope.pub_toggle_near_goal.publish($scope.msg_vision_main_data_on);
    }
    if (!e.checked) {
      checkedToggleNearGoal = 0;
      $scope.msg_vision_all_data_on.data = 0;
      $scope.exportClickedAll();
      $scope.variablechange = "NEAR GOAL OFF";
      console.log($scope.msg_vision_all_data_on);
      $scope.pub_toggle_near_goal.publish($scope.msg_vision_all_data_on);
    }
  }

  $scope.initToggleNearGoal = function () {
    $scope.toggleNearGoalSetter();
  };

  // Toggle Setter
  $scope.toggleNearGoalSetter = function () {
    let myToggle = angular.element(document.querySelector('#toggleNearGoal'));
    if ($scope.checkedToggleNearGoal) {
      myToggle.attr('checked', 'checked');
      $scope.state_near_goal = "NEAR GOAL ON";
    }
    else {
  ...
  $scope.sub_toggle_near_assist = new ROSLIB.Topic({
    ros: $scope.ros,
    name: "near_assist_toggle",
    messageType: "std_msgs/UInt8"
  });

  $scope.pub_toggle_near_assist = new ROSLIB.Topic({
    ros: $scope.ros,
    name: "near_assist_toggle",
    messageType: "std_msgs/UInt8"
  });

  $scope.sub_toggle_near_assist.subscribe((msg) => {
    let myToggle = angular.element(document.querySelector('#toggleNearAssist'));
    if (Number(msg.data) === 1) {
      myToggle.attr('checked', 'checked');
      $scope.state_near_assist = "NEAR ASSIST ON";
      $scope.checkedToggleNearAssist = true;
    } else {
      myToggle.removeAttr('checked');
      $scope.state_near_assist = "NEAR ASSIST OFF";
      $scope.checkedToggleNearAssist = false;
    }
  });

  $scope.toggleNearAssist = function (e) {
    console.log(e.checked);
    if (e.checked) {
      checkedToggleNearAssist = 1;
      $scope.msg_vision_main_data_on.data = 1;
      $scope.exportClickedMain();
      $scope.variablechange = "NEAR ASSIST ON";
      console.log($scope.msg_vision_main_data_on);
      $scope.pub_toggle_near_assist.publish($scope.msg_vision_main_data_on);
    }
    if (!e.checked) {
      checkedToggleNearAssist = 0;
      $scope.msg_vision_all_data_on.data = 0;
      $scope.exportClickedAll();
      $scope.variablechange = "NEAR ASSIST OFF";
      console.log($scope.msg_vision_all_data_on);
      $scope.pub_toggle_near_assist.publish($scope.msg_vision_all_data_on);
    }
  }

  $scope.initToggleNearAssist = function () {
    $scope.toggleNearAssistSetter();
  };

  // Toggle Setter
  $scope.toggleNearAssistSetter = function () {
    let myToggle = angular.element(document.querySelector('#toggleNearAssist'));
    if ($scope.checkedToggleNearAssist) {
      myToggle.attr('checked', 'checked');
      $scope.state_near_assist = "NEAR ASSIST ON";
    }
    else {
