...
MainAppField.controller('FieldController', function ($scope, $interval) {
    ...
    $scope.PauseStartStream = function (index) {
        if ($scope.vision_stream[index]) {
            ...
            if (index == 0)
                $scope.frame_field_rgb = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/vision_field_thresh_main&type=ros_compressed";
            else if (index == 1)
                $scope.frame_field_binary = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/vision_field_raw_thresh_main&type=ros_compressed";
        }
    }
    console.log("field Controller Loaded");

    //FOR IMAGE ADDRESS SHOWING
    $scope.frame_image_0 = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/vision_field_thresh_main&type=ros_compressed";
    $scope.frame_image_1 = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/field_layer_total&type=ros_compressed";
    $scope.frame_image_2 = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/field_layer_1&type=ros_compressed";
    $scope.frame_image_3 = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/field_layer_2&type=ros_compressed";
    $scope.frame_image_4 = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/field_layer_3&type=ros_compressed";
