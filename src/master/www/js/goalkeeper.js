...
MainAppField.controller('FieldController', function ($scope, $interval) {
    ...
    $scope.PauseStartStream = function (index) {
        if ($scope.vision_stream[index]) {
            ...
            if (index == 0)
                $scope.frame_field_rgb = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/world_model/display/goalpost&type=ros_compressed";
            else if (index == 1)
                $scope.frame_field_binary = "http://" + window.location.hostname + ":9901" + "/stream?topic=" + $scope.ns + "/world_model/debug/goalpost&type=ros_compressed";
        }
    }
