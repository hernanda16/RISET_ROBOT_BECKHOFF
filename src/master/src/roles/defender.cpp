...
void Defender::PrepKickoffHomeDouble() const
{
    ...
    *robot_condition = 0;
    ...
    case 0:
        ...
        break;
    case 100:
        if (motion->ruckigToPoint3(FIELD_X_1_2, FIELD_Y_1_4 - 100, 90, 100, 90, NORMAL_OBSTACLE | NORMAL_OBSTACLE_BALL | NORMAL_OBSTACLE_FRIEND))
            game_sub_status_prep->value = 120;

        if (ball->status > 0 && pythagoras(ball->pos.x, ball->pos.y, FIELD_X_1_2, FIELD_Y_1_2) < 100 && motion->RobotDist2Point(FIELD_X_1_2, FIELD_Y_1_4 - 100) < 100)
            game_sub_status_prep->value = 120;
        break;
...
void Defender::HasBall() const
{
    ...
    case 10: {
        ...
        if (motion->RobotDist2Point(*pos_buffer_robot_has_ball[0], *pos_buffer_robot_has_ball[1]) > 220) {
            if (obstacle.status && final_pose->y > FIELD_Y_1_2 - 150 && bs_data->style != 'C' && bs_data->style != 'D' && bs_data->style != 'M' /*&& final_pose->y < FIELD_Y_1_2 + 250*/) {
