...
void Master::callbackSubscribeStm2pc(const iris_msgs::stm2pcConstPtr& msg)
{
    ...
    // printf("button : %d %d %d %d %d %d %d %d\n", stm2pc_data.button1_pressed,
    //     stm2pc_data.button2_pressed,
    //     stm2pc_data.button3_pressed,
    //     stm2pc_data.button4_pressed,
    //     stm2pc_data.button5_pressed,
    //     stm2pc_data.toggle1_pressed,
    //     stm2pc_data.toggle2_pressed,
    //     stm2pc_data.toggle3_pressed);
...
void Master::callbackSubscribeRobotData(const iris_msgs::data_robotsConstPtr& msg)
{
    ...
    for (uint8_t i = 1; i < msg->target_positioning_x.size(); i++) {
        ...
        shared_robot_data[i].target_positioning[1] = msg->target_positioning_y.at(i);
    }
}
...
void Master::keyboardHandler()
{
    if (kbhit() > 0) {
        ...
        case ';':
            kickerControl(&stm_data, THROTTLE_AIM, 8, 400);
            // buzzerControl(&stm_data, 10, 10);
            break;
        case ':':
            kickerControl(&stm_data, KICKER_POSITION_AIM, 10, 250);
            // buzzerControl(&stm_data, 10, 10);
            break;
        case '-':
        ...
        case '|':
            ...
            break;
        case ',':
            // shared_robot_data[3].robot_condition = 0;
            stm_data.force_dribble_control = 0;
            break;
        case '.':
            // shared_robot_data[3].robot_condition = 11;
...
void Master::transmitAll()
{
    ...
    if (last_time_kick - start_time_kick > 0.5)
...
void Master::playSelectorControl()
{
    ...
    {
        has_prep = 0;
    } else if (game_status.value > 98 && game_status.value < 117) // Prep away
    {
        has_prep = 2;
...
#define NASIONAL
...
#else
    case status_preparation_kickoff_home:
        if (bs_data.n_active_robot > 1 && bs_data.style != 'F') {
            if (role == 2) {
    ...
    case status_park:
        motion->newMotionToPoint(init_offset.x, init_offset.y, init_offset.theta, 150, 180);
        break;
    //-------------------------------------------------------------------
    case 'j':
    ...
    // printf("%.2f %.2f %.2f\n", final_pose.x, act_vel.x, feedback_vel.x);
}
