...
void Motion::init(pose2d_t* final_pose, vel2d_t* act_vel, vel2d_t* world_vel, ball_t* ball, pc2stm_t* stm_data, std::vector<point2d_t>* waypoints_ball, std::vector<point2d_t>* obstacle, std::vector<point2d_t>* obstacle_pcl)
{
    ...
    translation_profile_.max_velocity = 250; // 250 // 180
    translation_profile_.max_acceleration = 180; // 200 // 90
    ...
    translation_profile_with_ball.max_velocity = 170; // 150 // 170
    translation_profile_with_ball.max_acceleration = 180; // 130 //200
    translation_profile_with_ball.max_jerk = 500;

    //===================================================================

    angular_profile_.max_velocity = 210;
    angular_profile_.max_acceleration = 150;
    angular_profile_.max_jerk = 500;

    angular_profile_with_ball.max_velocity = 180;
    angular_profile_with_ball.max_acceleration = 90;
...
void Motion::manualMotion(float vel_x, float vel_y, float vel_theta, bool is_ruckig)
{
    ...
    if (ball->is_caught > 0) {
        if (polar_acc.r > acc_max * 0.8) {
            polar_acc.r = acc_max * 0.8;
        }
    } else {
    ...
    if (d_theta > 7.3 && ball->is_caught > 0) {
        d_theta = 7.3;
    } else if (d_theta < -7.3 && ball->is_caught > 0) {
        d_theta = -7.3;
    }
...
{
    // TODO : Make Config
    static PID linear_pid(2, 0, 0); // 1.8, 0, 0
    static PID angular_pid(2.5, 0, 0.5); // 1.8, 0, 0.5
...
int8_t Motion::ruckigToPoint3(float target_x, float target_y, float target_theta, uint16_t minmax_vel_linear, uint16_t minmax_vel_angular, uint8_t flags)
{
    ...
    if ((flags & NORMAL_OBSTACLE) == NORMAL_OBSTACLE || (flags & NORMAL_OBSTACLE_BALL) == NORMAL_OBSTACLE_BALL || (flags & NORMAL_OBSTACLE_FRIEND) == NORMAL_OBSTACLE_FRIEND) {
        ...
        if ((flags & NORMAL_OBSTACLE) == NORMAL_OBSTACLE)
            obs = obstacleChecking(arah_jalan, 60, 200, flags);
        ...
        if ((flags & INVERT_OBS) == INVERT_OBS) {
            obstacle_checking_t obs_full = obstacleChecking(arah_jalan, 180, 200, NORMAL_OBSTACLE);
            if (obs_full.status) {
                target_theta_baru = obs_full.angle - 180;
            } else if (obs.status) {
                target_theta_baru = obs.angle - 180;
            }
        }
...
int8_t Motion::ruckigToPoint(float target_x, float target_y, float target_theta, uint16_t minmax_vel_linear, uint16_t minmax_vel_angular, uint8_t flags)
{
    ...
    if (error_xy < 70 && fabs(error_angular) < 2)
        return 1;
...
{
    // TODO : Make Config
    static PID linear_pid(1.9, 0, 0); // 1.8, 0, 0 // 1.6, 0.001, 0 //1.87
    // static PID linear_pid(1.8, 0.001, 0); // 1.8, 0, 0 // 1.6, 0.001, 0 // asli 26 mei 2024
    static PID angular_pid(1.8, 0, 0); // 1.8, 0, 0.5
...
{
    static PID angular_pid(kp_angular, 0, 0); // 1.8, 0, 0
...
{
    // static PID linear_pid(1, 0, 0);
    // static PID angular_pid(3, 0, 0);
    // static PID arching_pid(1.8, 0, 0); // 1.3 0..0000001 0 // 2,0,0 //bagus 1.8 0.01 0

    static PID linear_pid(1, 0, 0);
    static PID angular_pid(2.5, 0, 0);
    static PID arching_pid(1, 0, 0); // 1.3 0..0000001 0 // 2,0,0 //bagus 1.8 0.01 0
    ...
    if (fabs(error_linear) < 30 && fabs(error_angular) < 12 && fabs(error_arching) < 20) {
        return 1;
    } else {
        return 0;
    }
...
uint8_t Motion::receiveBall(float vel_linear, float vel_angular)
{
    ...
    case 1:
        ...
        if (pythagoras(final_pose->x, final_pose->y, ball->pos.x, ball->pos.y) < 80) {
            ...
            recv_ball_state.value = 2;
        } else if (fabs(delta_error_angle) < 30 && ball_distance > 30) {
            printf("fabs(delta_error_angle) < 30 && ball_distance > 40\n");

            motionAroundPointAngle(last_ball_pos[0], last_ball_pos[1], ball_angle - 180, (atan2(ball->pos.y - last_ball_pos[1], ball->pos.x - last_ball_pos[0]) RAD2DEG) - 180, blocking_distance, 200, 360);
        } else {
            printf("else\n");
...
uint8_t Motion::receiveBall3(float default_x, float default_y, float vel_linear, float vel_angular)
{
    ...
    printf("status : %d predict : %.2f %.2f ball_status : %d size wp : %d\n", recv_ball_state.value, waypoint_terdekat_x, waypoint_terdekat_y, ball->status, waypoints_ball->size());
    recv_ball_state.reentry(0, 0.04);

    static uint16_t counter_gagal = 0;

    mutex_ball_waypoints.lock();
    switch (recv_ball_state.value) {
    case 0:
        if (ball->is_caught > 0) {
            recv_ball_state.value = 3;
        ...
        if (waypoints_ball->size() > 1 && (ball->status == 2 || ball->status == 1)) {
        ...
        if (ball->status == 0) {
            ...
            motionToPointRecvBall(final_pose->x, final_pose->y, ball->angle, 0, 3);
        }
    ...
    case 1:
        ...
        if (pythagoras(final_pose->x, final_pose->y, target_intercept_ball_x, target_intercept_ball_y) > 300) {
            motionToPointRecvBall(target_intercept_ball_x, target_intercept_ball_y, sudut_ke_bola, 10, 10, 1); // 50,90
        } else {
            // motionToPointRecvBall(target_intercept_ball_x, target_intercept_ball_y, arah_bola - 180, 250, 180, 1);
            motionToPointRecvBall(target_intercept_ball_x, target_intercept_ball_y, ball->angle, 200, 90, 1);
        }
...
uint8_t Motion::catchingBall(float vel_linear, float vel_angular, uint8_t flag)
{
    ...
    static float target_ball_x_predict;
    static float target_ball_y_predict;
    static float ball_magnitude;
    static float ball_direction;
    float minimum_distance_predict = FLT_MAX;
    ...
    ball_direction = atan2f(ball->vel.y, ball->vel.x) RAD2DEG;
    ...
    for (size_t i = 0; i < waypoints_ball->size(); i++) {
        ...
        float ball_predicted_x = waypoints_ball->at(i).x;
        float ball_predicted_y = waypoints_ball->at(i).y;
        if (ball_predicted_x < FIELD_X_0 + 100 || ball_predicted_x > FIELD_X_1 - 100 || ball_predicted_y < FIELD_Y_0 + 100 || ball_predicted_y > FIELD_Y_1 - 100) {
            continue;
        }

        float jarak_bola_ke_titik_target = pythagoras(waypoints_ball->at(0).x, waypoints_ball->at(0).y, ball_predicted_x, ball_predicted_y);
        float t_bola = jarak_bola_ke_titik_target / ball_magnitude;

        float jarak_robot_ke_titik_target = pythagoras(final_pose->x, final_pose->y, ball_predicted_x, ball_predicted_y);
        float t_robot = jarak_robot_ke_titik_target / (vel_linear * 1.1);

        if (t_robot < t_bola) {
            minimum_distance_predict = jarak_robot_ke_titik_target;
            target_ball_x_predict = ball_predicted_x;
            target_ball_y_predict = ball_predicted_y;
            // predicted = 1;
        }
    }

    printf("KEJARBOLA: %d %d %d || %.2f %.2f (%.2f %.2f %d) (%.2f %.2f)\n", catch_ball_state.value, ball->status, ball->is_caught, ball->distance, fabs(error_angle),
        target_ball_x_predict, target_ball_y_predict, predicted, ball_magnitude, ball_direction);
    ...
    case 0:
        ...
        if (ball->distance < 80 && fabs(error_angle) < 3 && ball->status > 0) {
        ...
        break;
    case 1:
        if (ball->distance < 100 && fabs(error_angle) > 11 && ball->status > 0) {
            catch_ball_state.value = 3;
        } else if (ball->distance < 100 && fabs(error_angle) > 11 && ball->status == 0) {
    ...
    case 2:
        ...
        if ((predicted == 0 && ball->distance < 80 && fabs(error_angle) < 3 && ball->status > 0) || (predicted == 1 && ball->distance < 80 && fabs(error_angle) < 5 && ball->status > 0 && deltaAngle(ball_direction, RobotAngletoBall() - 180) < 15)) {
            catch_ball_state.value = 1;
        ...
        if (predicted == 0)
            motionToPoint(target_ball_x, target_ball_y, ball->angle, vel_linear, vel_angular, flag);
        else
            motionToPoint(target_ball_x_predict, target_ball_y_predict, ball->angle, vel_linear * 1.1, vel_angular * 1.1, flag);
    ...
    case 3:
        ...
        motionToPoint(ball->pos.x, ball->pos.y, ball->angle, vel_linear, vel_angular, flag);
        break;
    }
    ...
    printf("OBSTACLE : %d %.2f %.2f %.2f\n", obs_depan.status, obs_depan.distance, obs_depan.angle, ball->distance);
...
obstacle_checking_t Motion::obstacleCheckNearestTo(float theta, float tolerance, float dist, float to_x, float to_y)
{
    float min_dist = FLT_MAX;
    obstacle_checking_t ret_temp;
    ...
    for (size_t obs_it = 0; obs_it < obstacle_pcl->size(); obs_it++) {
        ...
        float jarak_target_to_obs = pythagoras(to_x, to_y, obstacle_pcl->at(obs_it).x, obstacle_pcl->at(obs_it).y);
        ...
        // printf("OBSTACLE: %.2f < %.2f | %.2f < %.2f | %.2f < %.2f | %d\n", jarak_target_to_obs, min_dist, jarak_robot_ke_obs, dist, fabs(delta_sudut_), tolerance, ret_temp.status);
        if (jarak_target_to_obs < min_dist && jarak_robot_ke_obs < dist && fabs(delta_sudut_) < tolerance) {
            ret_temp.status = 1;
            ret_temp.distance = jarak_target_to_obs;
            ret_temp.angle = sudut_robot_ke_obs;
            ret_temp.pos_x = obstacle_pcl->at(obs_it).x;
            ret_temp.pos_y = obstacle_pcl->at(obs_it).y;

            // printf("OBSTACLE: %.2f %.2f %.2f %.2f\n", ret_temp.pos_x, ret_temp.pos_y, ret_temp.distance, ret_temp.angle);

            min_dist = jarak_target_to_obs;
        }
    }
...
{
    static PID angular_pid(2.75, 0, 0.01);
...
uint8_t Motion::hadapGawangVisionPivot(float target_x, float target_y, uint16_t radius_point, uint16_t minmax_vel_linear, uint16_t minmax_vel_angular, uint8_t flag)
{
    static PID linear_pid(1, 0, 0);
    static PID angular_pid(2.5, 0, 0);
    static PID arching_pid(1, 0, 0); // 1.3 0..0000001 0 // 2,0,0 //bagus 1.8 0.01 0
    ...
    error_linear = sqrtf(error_x * error_x + error_y * error_y) - radius_point;
    error_angular = *target_goal_vision - final_pose->theta - 180 * ((flag & INVERT) == INVERT);

    (error_angular > 180) ? error_angular -= 360 : (error_angular < -180) ? error_angular += 360
                                                                          : error_angular;

    float error_arching = *target_goal_vision - angleToTarget(target_x, target_y);
    (error_arching > 180) ? error_arching -= 360 : (error_arching < -180) ? error_arching += 360
                                                                          : error_arching;
    ...
    if (fabs(error_linear) < 13 && fabs(error_angular) < 7 && fabs(error_arching) < 13) {
        return 1;
    } else {
        return 0;
    }
