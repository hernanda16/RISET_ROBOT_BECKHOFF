...
void callbackRoutine(const ros::TimerEvent& event)
{
    ...
    circle(camera_mask, Point(center_cam_x, center_cam_y), inner_mask_radius, Scalar(0), FILLED, LINE_8);
    ...
    dilate(buffer_ball, buffer_ball, getStructuringElement(MORPH_ELLIPSE, Size(8, 8)));
    ...
    dilate(threshold_obs, threshold_obs, getStructuringElement(MORPH_ELLIPSE, Size(2, 2)));

    bitwise_not(threshold_obs, threshold_obs);
    bitwise_and(threshold_obs, frame_field_hull, threshold_obs);

    erode(threshold_obs, threshold_obs, getStructuringElement(MORPH_ELLIPSE, Size(4, 4)));
...
#ifdef REGRESSION
    {
        Mat line_regression = Mat::zeros(frame_field_displayed.size(), CV_8UC3);
        for (float angle = 0; angle < 360; angle += 10) {
            int16_t pixel_x = center_cam_x + 180 * cos(angle DEG_TO_RAD);
            int16_t pixel_y = center_cam_y - 180 * sin(angle DEG_TO_RAD);
            line(line_regression, Point(center_cam_x, center_cam_y), Point(pixel_x, pixel_y), Scalar(255, 0, 255, 255), 1, LINE_8);
        }
        addWeighted(frame_field_displayed, 1, line_regression, 0.8, 0, frame_field_displayed);
    }
#endif
    //=============================================
    /**--------------------------------------------
